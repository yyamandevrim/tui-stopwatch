...
function splitToJsonEntry(split: SplitEntry) {
  return {
    index: split.index,
...
function buildCsv(splits: SplitEntry[]): string {
...
function buildTxt(splits: SplitEntry[]): string {
  if (splits.length === 0) {
...
export async function exportSplits(splits: SplitEntry[], cwd: string = process.cwd()): Promise<ExportResult> {
